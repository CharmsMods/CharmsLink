import { encodePng16Rgba } from './png16.mjs';
import { parseDngBuffer, decodeDngRaster } from './dngTiff.mjs';
import { buildDngPrepareHash, getDngPresetForProbe, normalizeDngDevelopParams } from './dngDevelopShared.mjs';
...
function applyWhiteBalance(rgb, parsed, params) {
    ...
    if (params.dngWhiteBalanceMode === 'custom') {
        const temperatureRgb = temperatureToRgb(params.dngTemperature);
        const tint = clamp(Number(params.dngTint || 0) / 100, -1, 1);
        gains = [
            1 / Math.max(1e-6, temperatureRgb[0]),
            1 / Math.max(1e-6, temperatureRgb[1] * (1 + tint * 0.35)),
            1 / Math.max(1e-6, temperatureRgb[2])
        ];
    } else if (asArray(parsed?.probe?.asShotNeutral).length >= 3) {
        const neutral = asArray(parsed.probe.asShotNeutral).slice(0, 3).map((value) => Math.max(1e-6, Number(value) || 1));
...
function applyToneAndDetail(rgb, width, height, params) {
...
export async function prepareDngBuffer(buffer, params = null, options = {}) {
    const { parsed, signature } = await getParsed(buffer);
    const raster = await getRaster(parsed, signature);
