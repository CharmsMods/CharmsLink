...
import { createWorkspaceUI } from '../ui/workspaces/index.js';
...
import { arrayBufferToDataUrl, blobToDataUrl, bytesToHex, dataUrlToBlob, fileToDataUrl, sampleHashString } from '../utils/dataUrl.js';
import {
...
function resolveLibraryProjectType(entry) {
...
function createLibraryProjectListPayload(record) {
    ...
    if (!payload || typeof payload !== 'object') return payload;
    const projectType = resolveLibraryProjectType(record);

    const nextPayload = { ...payload };

    if (projectType === 'studio') {
        if (nextPayload.source && typeof nextPayload.source === 'object') {
            nextPayload.source = {
                ...nextPayload.source,
                imageData: null,
                rawData: null
            };
            if (isDngSource(nextPayload.source)) {
                nextPayload.source = normalizeEditorSource({
                    ...stripDngSourceRawPayload(nextPayload.source),
                    imageData: null,
                    dng: {
                        ...nextPayload.source.dng,
                        lastPreparedParamsHash: '',
                        lastPreparedAt: 0
                    }
                });
    ...
    if (projectType === 'stitch' && Array.isArray(nextPayload.inputs)) {
        nextPayload.inputs = nextPayload.inputs.map((input) => {
            if (!input || typeof input !== 'object') return input;
            return { ...input, imageData: null, rawData: null };
        });
    }

    if ((projectType === 'composite') && Array.isArray(nextPayload.layers)) {
        nextPayload.layers = nextPayload.layers.map((layer) => {
            if (!layer || typeof layer !== 'object') return layer;
            const stripped = { ...layer };
            if (stripped.source && typeof stripped.source === 'object') {
                stripped.source = { ...stripped.source, imageData: null, rawData: null };
            }
            return stripped;
        });
    }
...
export async function bootstrapStudioApp({ root = document.getElementById('app') } = {}) {
    ...
    function logFatalBootstrapIssue(message, details = {}) {
        if (!message) return;
        logger.error('app.bootstrap', PROCESS_LABELS['app.bootstrap'], message, {
            dedupeKey: `fatal:${message}`,
            dedupeWindowMs: 200,
            ...details
        });
    }
    ...
    window.addEventListener('error', (event) => {
        const errorMessage = event?.error?.message || event?.message || 'An uncaught renderer error occurred.';
        logFatalBootstrapIssue(errorMessage, {
    ...
    window.addEventListener('unhandledrejection', (event) => {
        ...
        const message = reason?.message || String(reason || 'An unhandled promise rejection occurred.');
        logFatalBootstrapIssue(message, {
    ...
    const appContext = createAppContext({
        ...
        getEngine: () => engine,
        getStitchEngine: () => stitchEngine,
        getProjectAdapters: () => projectAdapters,
    ...
    function registerWorkerDomains(context) {
        void context;
    ...
    registerWorkerDomains(appContext);
    ...
    async function upsertDerivedStudioAsset(projectRecord, options = {}) {
        if (!projectRecord || resolveLibraryProjectType(projectRecord) !== 'studio') return null;
        ...
        studioProjectPreviewBackfillPromise = (async () => {
            ...
            try {
                for (let index = 0; index < sourceProjects.length; index += 1) {
                    ...
                    if (resolveLibraryProjectType(projectRecord) !== 'studio') continue;
    ...
    async function persistEmbeddedStudioPayloadToLibrary(rawPayload, options = {}) {
        const existingRecord = options.existingRecord && resolveLibraryProjectType(options.existingRecord) === 'studio'
        ...
        try {
            return await getLibraryProjectRecordsByCursor({
                filterRecord(entry) {
                    ...
                    if (projectType && resolveLibraryProjectType(entry) !== projectType) return false;
                ...
                mapRecord(entry) {
                    return {
                        id: entry.id,
                        name: entry.name,
                        projectType: resolveLibraryProjectType(entry)
                    };
                }
            });
        } catch (_error) {
    ...
    function createSettingsActions(context) {
        void context;
        return {
        getState() {
        ...
        async runLibraryMaintenance(kind = '') {
            ...
            try {
                ...
                if (maintenanceKind === 'purge-rendered-previews') {
                    ...
                    const studioProjects = projectRecords.filter((entry) => resolveLibraryProjectType(entry) === 'studio');
    ...
    function createNavigationActions(context) {
        void context;
        return {
    ...
    function createCompositeActions(context) {
        void context;
        return {
        ...
        async listCompositeSourceProjects() {
            ...
            const projects = (await getAllLibraryProjectRecords())
                .filter((entry) => resolveLibraryProjectType(entry) === 'studio')
        ...
        async addCompositeEditorProjects(ids = []) {
            ...
            try {
                ...
                for (let index = 0; index < records.length; index += 1) {
                    ...
                    if (!record || resolveLibraryProjectType(record) !== 'studio') continue;
        ...
        async replaceCompositeLayerWithEditorProject(layerId = null, libraryId = null) {
            ...
            try {
                ...
                if (!record || resolveLibraryProjectType(record) !== 'studio') {
        ...
        async updateOriginalEditorProjectFromComposite() {
            ...
            try {
                ...
                if (!existingRecord || resolveLibraryProjectType(existingRecord) !== 'studio') {
    ...
    function createStitchActions(context) {
        void context;
        return {
    ...
    function createLibraryActions(context) {
        void context;
        return {
            ...
            try {
                return await getLibraryProjectRecordsByCursor({
                    mapRecord(entry) {
                        ...
                        const projectType = resolveLibraryProjectType(entry);
                        return {
                            id: entry.id,
                            kind: entry.kind,
                            ...
                            hoverSource: normalizeLibraryHoverSource(entry.hoverSource || projectMeta.hoverSource || null),
                            ...
                            renderWidth: Number(entry.renderWidth || 0),
                            renderHeight: Number(entry.renderHeight || 0)
                        };
                    }
                });
            } catch (error) {
                console.error(error);
                logProcess('error', 'library.sync', error?.message || 'Could not read Library projects from IndexedDB.');
                return [];
            }
        },
        async prepareLibraryProjectsForExport(projects = []) {
            ...
            for (const project of sourceProjects) {
                ...
                if (resolveLibraryProjectType(baseRecord) === 'studio') {
        ...
        async getLibraryAssets() {
            try {
                return await getAllLibraryAssetRecords();
            } catch (error) {
                console.error(error);
                logProcess('error', 'library.assets', error?.message || 'Could not read Library assets from IndexedDB.');
                return [];
            }
        },
        async getLibraryTagCatalog() {
            try {
                ...
                console.error(error);
                logProcess('error', 'library.sync', error?.message || 'Could not read the Library tag catalog.');
                return [];
            }
        },
        ...
        async deleteLibraryProject(id) {
            ...
            const projectType = resolveLibraryProjectType(existing);
        ...
        async processLibraryPayloads(payloadsInput, filenames, onProgress = null) {
            const state = store.getState();
            const existingProjects = await getAllLibraryProjectRecords();
            const knownFingerprints = new Set(
                existingProjects.map((entry) => `${resolveLibraryProjectType(entry)}:${makeProjectFingerprint(entry.payload)}`)
            );
            let savedCount = 0;
            let updatedCount = 0;
            let skippedDuplicateCount = 0;
            ...
            logProcess('active', 'library.import', `Rendering ${payloadsInput.length} imported project payload${payloadsInput.length === 1 ? '' : 's'} into the Library database.`);
            setNotice(`Rendering ${payloadsInput.length} variants to Library DB...`, 'info', 0);

            onProgress?.({ phase: 'start', total: payloadsInput.length });

            for (let i = 0; i < payloadsInput.length; i++) {
                onProgress?.({ phase: 'progress', count: i, total: payloadsInput.length, filename: filenames[i] });

                try {
                    const entry = payloadsInput[i];
                    const rawState = normalizeLegacyDocumentPayload(
                    ...
                    try {
                        ...
                        console.warn('[Library] Could not restore Editor engine after import error.', _restoreError);
                    }
                }

                onProgress?.({ phase: 'progress', count: i + 1, total: payloadsInput.length, filename: filenames[i] });
            ...
            onProgress?.({ phase: 'complete', total: payloadsInput.length });
            ...
            return {
                ...
                skippedDuplicateCount,
                rejectedCount,
                totalRequested: payloadsInput.length,
                importedTags,
                importedProjectIds
            };
        },
    ...
    function createThreeDActions(context) {
        void context;
        return {
    ...
    function createAppActions(context) {
        return Object.assign(
            ...
            createCompositeActions(context),
            createEditorActions(context),
            createStitchActions(context),
            createLibraryActions(context),
            createThreeDActions(context)
        );
    }

    const actions = createAppActions(appContext);
    appContext.merge({ actions });

    function createProjectAdapters(context) {
        void context;
        return [
        {
            type: 'studio',
            ...
            async prepareImportedProject(rawPayload) {
                ...
                const baseResolution = getEditorCanvasResolution(payload);
                let blob = null;
                if (payload.preview?.imageData) {
                    try {
                        blob = await dataUrlToBlob(payload.preview.imageData);
                    } catch (_error) {
                        blob = null;
                    }
                }
                return {
                    ...
                    summary: {
                        ...
                        sourceArea: Number(baseResolution.width || 0) * Number(baseResolution.height || 0),
                        sourceCount: hasPersistableEditorSource(payload.source) ? 1 : 0,
                        renderWidth: Number(baseResolution.width || 0),
                        renderHeight: Number(baseResolution.height || 0)
                    }
                };
            },
    ...
    projectAdapters = createProjectAdapterRegistry(createProjectAdapters(appContext));
    appContext.merge({ projectAdapters });

    function wireStoreRendering(context) {
        const boundStore = context.getStore();
        let lastSubscribedSection = boundStore.getState().ui.activeSection;
        let pendingEditorRender = false;
        let pendingCompositeRender = false;
        let pendingStitchRender = false;

        boundStore.subscribe((state, meta) => {
        ...
        view.render(boundStore.getState());
        engine.attachRefs(view.getRenderRefs());
    }
    ...
    if (activeSettings.library.autoLoadOnStartup) {
        if (store.getState().ui.activeSection === 'library') {
            logProcess('info', 'settings.library', 'Priming the Library on startup because Library auto-load is enabled and Library is the active section.', {
                dedupeKey: 'settings-library-autoload-active',
                dedupeWindowMs: 1500
            });
            view.primeLibrary?.();
        } else {
            logProcess('info', 'settings.library', 'Deferred background Library priming on startup because another section opened first.', {
                dedupeKey: 'settings-library-autoload-deferred',
                dedupeWindowMs: 1500
            });
        }
    }
