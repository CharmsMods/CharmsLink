...
const DNG_CONTROL_HELP = Object.freeze({
    dngPreset: 'Starts from a camera-aware default bundle. Auto usually picks the safest starting point from the file metadata.',
    dngInterpretationMode: 'Tells the renderer whether the file should be treated as linear RGB or still as mosaic sensor data. Leave this on Auto unless the preview clearly looks wrong.',
    dngAutoBlackLevel: 'Uses the file-reported sensor black level. Turn this off only when the metadata is wrong and you need to set the floor manually.',
    dngBlackLevel: 'Manual sensor floor before normalization. Raising it deepens blacks. Lowering it opens more shadow detail and more noise.',
    dngAutoWhiteLevel: 'Uses the file-reported highlight ceiling. Turn this off only when highlights clip too early or the file metadata is unreliable.',
    dngWhiteLevel: 'Manual highlight ceiling before tone shaping. Higher values preserve highlights. Lower values brighten the image but clip sooner.',
    dngApplyLinearization: 'Applies the camera linearization lookup table before black and white normalization. Leave this on when the file includes one.',
    dngApplyOrientation: 'Applies the DNG orientation tag and default display rotation. Turn this off if the preview comes in rotated or flipped incorrectly.',
    ...
    dngExposure: 'Adds or removes overall exposure in stops before the later tone controls.',
    dngHighlightRecovery: 'Compresses bright highlights so clipped channels roll off more gracefully.',
    dngToneMappingAmount: 'Adds a broader highlight shoulder for a more display-ready preview.',
    dngDenoiseMode: 'Auto uses the preset denoise path. Manual uses your sliders. Off bypasses denoise entirely.',
...
function helpAttr(helpText) {
    return helpText ? ` data-help="${escapeHtml(helpText)}"` : '';
}
...
function rangeRow(instance, key, label, min, max, step = 1, fallback = 0, hidden = false, helpText = '') {
    const value = instance.params[key] ?? fallback;
    return `
        <div class="control-row control-row-range ${hidden ? 'is-hidden' : ''}"${helpAttr(helpText)}>
...
function selectControlRow(instance, key, label, options, selectedValue, extraAttributes = '', helpText = '') {
    return `
        <div class="control-row"${helpAttr(helpText)}>
            <div class="control-row-top"><label>${label}</label></div>
            <select data-control-instance="${instance.instanceId}" data-control-key="${key}" ${extraAttributes}>
                ${(options || []).map((option) => `<option value="${option.value}" ${String(option.value) === String(selectedValue) ? 'selected' : ''} ${option.disabled ? 'disabled' : ''}>${option.label}</option>`).join('')}
            </select>
        </div>
    `;
}

function checkboxRow(instance, key, label, checked, extraAttributes = '', helpText = '') {
    return `<label class="check-row"${helpAttr(helpText)}><input type="checkbox" ${checked ? 'checked' : ''} data-control-instance="${instance.instanceId}" data-control-key="${key}" ${extraAttributes}><span>${label}</span></label>`;
}

function renderDngDevelopEditor(instance, state) {
        ...
        return `
            <div class="custom-layer-editor">
                <section class="custom-group">
                    <div class="panel-heading">DNG Develop</div>
                    <div class="empty-inline">Load a DNG source to use this layer.</div>
                </section>
            </div>
        `;
    }
    ...
    const params = normalizeDngDevelopParams(instance.params || {}, probe);
    const warningList = [...new Set([...(probe.warnings || []), ...(source.dng?.warnings || [])].filter(Boolean))];
    const fidelityLabel = source.dng?.fidelity === 'supported'
        ? 'Supported'
        : source.dng?.fidelity === 'unsupported'
            ? DNG_UNSUPPORTED_TEXT
            : 'Partial Fidelity';
    ...
    const supportsGainMapCorrections = !!probe.supportsGainMapCorrections;
    const supportsDisplayP3 = !!params.dngApplyCameraMatrix;
    const whiteBalanceUsesOffsets = params.dngWhiteBalanceMode !== 'custom';
    const temperatureLabel = whiteBalanceUsesOffsets ? 'Temperature Shift' : 'Temperature';
    const tintLabel = whiteBalanceUsesOffsets ? 'Tint Shift' : 'Tint';
    ...
    const interpretationOptions = [
        ...
        { value: 'bayer', label: 'Bayer Mosaic' },
        { value: 'quad', label: 'Quad / Nona Mosaic', disabled: !supportsRemosaic },
        { value: 'interleaved', label: 'Interleaved', disabled: !supportsRemosaic }
    ];
    const workingSpaceOptions = [
        { value: 'srgb', label: 'sRGB' },
        { value: 'display-p3', label: supportsDisplayP3 ? 'Display P3' : 'Display P3 (Needs Camera Matrix)', disabled: !supportsDisplayP3 && params.dngWorkingSpace !== 'display-p3' },
        { value: 'linear', label: 'Linear' }
    ];
    ...
    return `
        <div class="custom-layer-editor dng-develop-editor">
            ...
            <section class="custom-group">
                ...
                <div class="info-banner is-data"><span>Dimensions</span><strong>${formatNumber(probe.width || source.width)} x ${formatNumber(probe.height || source.height)}</strong></div>
                <div class="info-banner is-data"><span>Bit Depth</span><strong>${formatNumber(probe.bitDepth || 0)}-bit</strong></div>
                <div class="info-banner is-data"><span>Compression</span><strong>${escapeHtml(probe.compressionLabel || 'Unknown')}</strong></div>
                <div class="info-banner is-data"><span>Detected Preset</span><strong>${escapeHtml(getDngPresetLabel(probe.preset || source.dng?.preset || params.dngPreset))}</strong></div>
                <div class="info-banner is-data"><span>Fidelity</span><strong>${escapeHtml(fidelityLabel)}</strong></div>
                ${warningList.map((warning) => `<div class="info-banner">${escapeHtml(warning)}</div>`).join('')}
            </section>
            <section class="custom-group">
                <div class="panel-heading">Interpretation</div>
                ${selectControlRow(instance, 'dngPreset', 'Preset Override', [
                    { value: 'auto', label: 'Auto' },
                    { value: 'samsung-expert-raw-linear', label: 'Samsung Expert RAW Linear' },
                    { value: 'samsung-pro-mode-mosaic', label: 'Samsung Pro Mode Mosaic' },
                    { value: 'quad-bayer-mosaic', label: 'Quad Bayer Mosaic' },
                    { value: 'interleaved-mosaic', label: 'Interleaved Mosaic' },
                    { value: 'generic-linear', label: 'Generic Linear DNG' },
                    { value: 'generic-mosaic', label: 'Generic Mosaic DNG' }
                ], params.dngPreset, '', DNG_CONTROL_HELP.dngPreset)}
                ${selectControlRow(instance, 'dngInterpretationMode', 'Interpretation Mode', interpretationOptions, params.dngInterpretationMode, '', DNG_CONTROL_HELP.dngInterpretationMode)}
                ${supportsRemosaic ? '' : '<div class="info-banner">This source is a standard Bayer or linear DNG, so quad/interleaved remosaic modes are not applicable here.</div>'}
            </section>
            <section class="custom-group">
                <div class="panel-heading">Normalize</div>
                ${checkboxRow(instance, 'dngAutoBlackLevel', 'Auto Black Level', params.dngAutoBlackLevel, '', DNG_CONTROL_HELP.dngAutoBlackLevel)}
                ${rangeRow(instance, 'dngBlackLevel', 'Manual Black Level', -65535, 65535, 1, 0, params.dngAutoBlackLevel, DNG_CONTROL_HELP.dngBlackLevel)}
                ${checkboxRow(instance, 'dngAutoWhiteLevel', 'Auto White Level', params.dngAutoWhiteLevel, '', DNG_CONTROL_HELP.dngAutoWhiteLevel)}
                ${rangeRow(instance, 'dngWhiteLevel', 'Manual White Level', 1, 65535, 1, 65535, params.dngAutoWhiteLevel, DNG_CONTROL_HELP.dngWhiteLevel)}
                ${checkboxRow(instance, 'dngApplyLinearization', `Apply Linearization Table${hasLinearization ? '' : ' (Not Present)'}`, params.dngApplyLinearization, hasLinearization ? '' : 'disabled', DNG_CONTROL_HELP.dngApplyLinearization)}
                ${checkboxRow(instance, 'dngApplyOrientation', `Apply Orientation${supportsOrientation ? '' : ' (Already Upright)'}`, params.dngApplyOrientation, supportsOrientation ? '' : 'disabled', DNG_CONTROL_HELP.dngApplyOrientation)}
            </section>
            <section class="custom-group">
                <div class="panel-heading">Remosaic / Demosaic</div>
                ${selectControlRow(instance, 'dngRemosaicMode', 'Remosaic', [
                    { value: 'auto', label: 'Auto' },
                    { value: 'off', label: 'Off' },
                    { value: 'on', label: 'On' }
                ], params.dngRemosaicMode, supportsRemosaic ? '' : 'disabled', DNG_CONTROL_HELP.dngRemosaicMode)}
                ${selectControlRow(instance, 'dngDemosaicQuality', 'Demosaic Quality', [
                    { value: 'fast', label: 'Fast' },
                    { value: 'high', label: 'High' }
                ], params.dngDemosaicQuality, '', DNG_CONTROL_HELP.dngDemosaicQuality)}
            </section>
            <section class="custom-group">
                <div class="panel-heading">Color / WB</div>
                ${selectControlRow(instance, 'dngWhiteBalanceMode', 'White Balance', [
                ...
                ${supportsDisplayP3 ? '' : '<div class="info-banner">Display P3 needs the camera-matrix transform path. With it off, the preview uses the safer non-matrix color path.</div>'}
                ${selectControlRow(instance, 'dngWorkingSpace', 'Working Space', workingSpaceOptions, params.dngWorkingSpace, '', DNG_CONTROL_HELP.dngWorkingSpace)}
            </section>
            <section class="custom-group">
                <div class="panel-heading">Tone</div>
                ${rangeRow(instance, 'dngExposure', 'Exposure', -10, 10, 0.1, 0, false, DNG_CONTROL_HELP.dngExposure)}
                ${rangeRow(instance, 'dngHighlightRecovery', 'Highlight Recovery', 0, 100, 1, 22, false, DNG_CONTROL_HELP.dngHighlightRecovery)}
                ${rangeRow(instance, 'dngToneMappingAmount', 'Tone Mapping Amount', 0, 100, 1, 18, false, DNG_CONTROL_HELP.dngToneMappingAmount)}
            </section>
            <section class="custom-group">
                <div class="panel-heading">Noise / Detail</div>
                ${selectControlRow(instance, 'dngDenoiseMode', 'Denoise Mode', [
                    { value: 'auto', label: 'Auto' },
                    { value: 'off', label: 'Off' },
                    { value: 'manual', label: 'Manual' }
                ], params.dngDenoiseMode, '', DNG_CONTROL_HELP.dngDenoiseMode)}
                ${rangeRow(instance, 'dngDenoiseStrength', 'Denoise Strength', 0, 100, 1, 14, params.dngDenoiseMode === 'off', DNG_CONTROL_HELP.dngDenoiseStrength)}
                ${rangeRow(instance, 'dngDetailPreservation', 'Detail Preservation', 0, 100, 1, 62, params.dngDenoiseMode === 'off', DNG_CONTROL_HELP.dngDetailPreservation)}
                ${rangeRow(instance, 'dngSharpenAmount', 'Sharpen Amount', 0, 100, 1, 10, false, DNG_CONTROL_HELP.dngSharpenAmount)}
            </section>
            <section class="custom-group">
...
function renderControl(instance, control, state, layerDef) {
    ...
    switch (control.type) {
        ...
        case 'dngDevelopEditor':
            return renderDngDevelopEditor(instance, state);
        default:
            return '';
    }
}
        ...
        <div class="panel-section controls-section">
            <div class="custom-layer-editor">
                ...
                <section class="custom-group">
                    ...
                    <div class="button-cluster">
                        <button type="button" class="mini-button ${String(state.document.export?.pngBitDepth || '8-bit') === '8-bit' ? 'is-active' : ''}" data-action="editor-export-bit-depth" data-value="8-bit">8-bit PNG</button>
                        <button type="button" class="mini-button ${String(state.document.export?.pngBitDepth || '8-bit') === '16-bit' ? 'is-active' : ''}" data-action="editor-export-bit-depth" data-value="16-bit" ${isDngSource(source) ? '' : 'disabled'}>16-bit PNG</button>
                    </div>
                    <div class="info-banner">${isDngSource(source) ? 'DNG-backed projects can export through the 16-bit worker path. Non-DNG projects stay on 8-bit PNG export.' : '16-bit PNG export is only available for DNG-backed Editor projects.'}</div>
                </section>
                <section class="custom-group">
    ...
    return `
        <header class="toolbar">
            <div class="toolbar-cluster">
                ...
                <button type="button" class="toolbar-button" data-action="composite-add-triangle">Add Triangle</button>
            </div>
            <div class="toolbar-cluster toolbar-state-actions">
...
export function createWorkspaceUI(root, registry, actions, extras = {}) {
    root.innerHTML = `
        <div class="app-shell">
            ...
            <div class="workspace-shell" id="workspaceShell">
                ...
                <section class="workspace-center">
                    <div class="preview-shell" id="previewShell">
                        ...
                        <div class="preview-render-status" id="previewRenderStatus" aria-live="polite">
                            <span class="preview-render-status-spinner" aria-hidden="true"></span>
                            <div class="preview-render-status-copy">
                                <strong id="previewRenderStatusTitle">Rendering</strong>
                                <span id="previewRenderStatusMessage">Updating the DNG preview...</span>
                            </div>
                        </div>
            ...
            <input id="imageInput" type="file" accept="image/*,.dng" hidden>
            ...
            <div id="hoverHelpTooltip" class="hover-help-tooltip" hidden></div>
    ...
    const refs = {
        ...
        previewRenderStatus: root.querySelector('#previewRenderStatus'),
        previewRenderStatusTitle: root.querySelector('#previewRenderStatusTitle'),
        previewRenderStatusMessage: root.querySelector('#previewRenderStatusMessage'),
        ...
        hoverHelpTooltip: root.querySelector('#hoverHelpTooltip'),
    ...
    function clearHoverHelpTimer() {
        if (hoverHelpTimer) {
            clearTimeout(hoverHelpTimer);
            hoverHelpTimer = null;
        }
    }

    function hideHoverHelpTooltip() {
        clearHoverHelpTimer();
        hoverHelpTarget = null;
        if (!refs.hoverHelpTooltip) return;
        refs.hoverHelpTooltip.hidden = true;
        refs.hoverHelpTooltip.classList.remove('is-visible');
        refs.hoverHelpTooltip.textContent = '';
    }

    function positionHoverHelpTooltip(target) {
        if (!refs.hoverHelpTooltip || !target?.isConnected) return;
        const rect = target.getBoundingClientRect();
        const tooltip = refs.hoverHelpTooltip;
        const maxWidth = Math.min(320, Math.max(220, window.innerWidth - 24));
        tooltip.style.maxWidth = `${maxWidth}px`;
        tooltip.style.left = `${Math.min(window.innerWidth - 12, Math.max(12, rect.left + (rect.width * 0.5)))}px`;
        tooltip.style.top = `${Math.max(16, rect.top - 12)}px`;
    }

    function showHoverHelpTooltip(target) {
        if (!refs.hoverHelpTooltip || !target?.isConnected) return;
        const helpText = String(target.dataset.help || '').trim();
        if (!helpText) return;
        hoverHelpTarget = target;
        refs.hoverHelpTooltip.textContent = helpText;
        refs.hoverHelpTooltip.hidden = false;
        refs.hoverHelpTooltip.classList.add('is-visible');
        positionHoverHelpTooltip(target);
    }

    function scheduleHoverHelpTooltip(target) {
        clearHoverHelpTimer();
        ...
        hoverHelpTarget = target;
        hoverHelpTimer = setTimeout(() => {
            if (hoverHelpTarget === target) {
    ...
    function refreshEditorPreviewInfoRefs() {
        ...
        refs.previewRenderStatus = root.querySelector('#previewRenderStatus');
        refs.previewRenderStatusTitle = root.querySelector('#previewRenderStatusTitle');
        refs.previewRenderStatusMessage = root.querySelector('#previewRenderStatusMessage');
    }
    ...
    const handleAction = (node) => {
        ...
        const actionMap = {
            ...
            'composite-add-triangle': actions.addCompositeTriangleLayer,
    ...
    root.addEventListener('pointerover', (event) => {
        const helpTarget = event.target instanceof Element ? event.target.closest('[data-help]') : null;
        if (!helpTarget || !root.contains(helpTarget)) return;
        const previousHelpTarget = event.relatedTarget instanceof Element ? event.relatedTarget.closest('[data-help]') : null;
        if (previousHelpTarget === helpTarget) return;
        scheduleHoverHelpTooltip(helpTarget);
    });

    root.addEventListener('pointerout', (event) => {
        const helpTarget = event.target instanceof Element ? event.target.closest('[data-help]') : null;
        if (!helpTarget) return;
        const nextHelpTarget = event.relatedTarget instanceof Element ? event.relatedTarget.closest('[data-help]') : null;
        if (nextHelpTarget === helpTarget) return;
        if (hoverHelpTarget === helpTarget) {
    ...
    root.addEventListener('pointermove', (event) => {
        if (!refs.toleranceTooltip && !refs.hoverHelpTooltip) return;
        if (refs.hoverHelpTooltip && !refs.hoverHelpTooltip.hidden && hoverHelpTarget?.isConnected) {
            positionHoverHelpTooltip(hoverHelpTarget);
        }
        const specCanvas = event.target.closest('.tolerance-spectrum-canvas');
        if (specCanvas && refs.toleranceTooltip) {
    ...
    root.addEventListener('scroll', () => hideHoverHelpTooltip(), true);
    ...
    function render(state) {
        ...
        if (isEditorActive) {
            ...
            if (!hasSource) {
                refs.previewRenderStatus?.classList.remove('is-active');
            }
    ...
    return {
        ...
        setEditorPreviewRenderStatus(payload = null) {
            if (!refs.previewRenderStatus) return;
            const active = !!payload?.active;
            refs.previewRenderStatus.classList.toggle('is-active', active);
            if (!active) return;
            if (refs.previewRenderStatusTitle) {
                refs.previewRenderStatusTitle.textContent = String(payload.title || 'Rendering');
            }
            if (refs.previewRenderStatusMessage) {
                refs.previewRenderStatusMessage.textContent = String(payload.message || 'Updating the DNG preview...');
            }
        },
        getEditorPreviewRenderStatus() {
            return {
                active: !!refs.previewRenderStatus?.classList.contains('is-active'),
                title: refs.previewRenderStatusTitle?.textContent || '',
                message: refs.previewRenderStatusMessage?.textContent || ''
            };
        },
