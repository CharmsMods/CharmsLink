...
export function isEmbeddedLinearRgbProbe(probe = null) {
    return String(probe?.classificationMode || '').toLowerCase() === 'linear'
        && Math.max(1, Number(probe?.samplesPerPixel) || 1) >= 3;
}
...
export function createDefaultDngDevelopParams() {
    return {
        ...
        dngSharpenAmount: 10,
        // Standalone opcode execution is not complete in the current build, so
        // keep the control off by default instead of implying it is active.
        dngApplyOpcodeCorrections: false,
...
function getPresetDefaults(probe = null) {
    ...
    if (preset === 'samsung-pro-mode-mosaic') {
        return {
            ...
            // approximate camera-matrix path or gain-map path enabled by default.
            dngApplyCameraMatrix: false,
            dngApplyGainMap: false
        };
    }
...
export function normalizeDngDevelopParams(params = null, probe = null) {
    ...
    if (!probe?.supportsOpcodeCorrections) {
        normalized.dngApplyOpcodeCorrections = false;
    }
    if (!probe?.supportsGainMapCorrections) {
        normalized.dngApplyGainMap = false;
    }
    return normalized;
}
...
export function computeDngDevelopGeometry(source = null, params = null) {
    const probe = source?.probe || source?.dng?.probe || source || {};
    const metadata = source?.metadata || {};
    const normalizedParams = normalizeDngDevelopParams(params, probe);
    const storedWidth = Math.max(1, Math.round(Number(source?.rawRaster?.width || probe?.storedWidth || probe?.width || source?.width || 1)));
    const storedHeight = Math.max(1, Math.round(Number(source?.rawRaster?.height || probe?.storedHeight || probe?.height || source?.height || 1)));
    const cropOrigin = toNumberArray(metadata.defaultCropOrigin?.length ? metadata.defaultCropOrigin : probe?.defaultCropOrigin);
    const cropSize = toNumberArray(metadata.defaultCropSize?.length ? metadata.defaultCropSize : probe?.defaultCropSize);
    const cropLeft = Math.max(0, Number(cropOrigin[0] || 0));
    const cropTop = Math.max(0, Number(cropOrigin[1] || 0));
    const cropWidth = Math.max(1, Math.min(storedWidth - cropLeft, Math.round(Number(cropSize[0] || probe?.width || storedWidth) || storedWidth)));
    const cropHeight = Math.max(1, Math.min(storedHeight - cropTop, Math.round(Number(cropSize[1] || probe?.height || storedHeight) || storedHeight)));
    const orientation = normalizedParams.dngApplyOrientation !== false
        ? Math.round(Number(probe?.orientation || source?.orientation || 1))
        : 1;
    const swap = shouldSwapOrientation(orientation);
    return {
        storedWidth,
        storedHeight,
        cropLeft,
        ...
        cropHeight,
        orientation,
        width: swap ? cropHeight : cropWidth,
        height: swap ? cropWidth : cropHeight
    };
}
