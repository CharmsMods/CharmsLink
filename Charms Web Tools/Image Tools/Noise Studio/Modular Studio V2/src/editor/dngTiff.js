import { decodeUtifJpegDng } from '../vendor/utif/module.js';
...
function readTagValue(reader, type, count, rawOffset, entryOffset) {
    ...
    if (type === 7) {
        return reader.slice(start, count);
    }
    const values = [];
...
function normalizeByteArray(value) {
    ...
    if (Array.isArray(value)) {
        return Uint8Array.from(value.map((entry) => clampPositiveInteger(entry, 0) & 0xff));
    }
    return new Uint8Array(0);
}

function usesPhaseAwareGainMapLayout(gainMaps = []) {
    if (!Array.isArray(gainMaps) || !gainMaps.length) return false;
    const validMaps = gainMaps.filter((entry) => entry && typeof entry === 'object');
    if (!validMaps.length) return false;
    if (validMaps.length === 1) return false;
    const hasOffsetVariants = new Set(
        validMaps.map((entry) => `${Number(entry.top || 0)}:${Number(entry.left || 0)}`)
    ).size > 1;
    const allDefaultPlane = validMaps.every((entry) => Number(entry.plane ?? 0) === 0);
    const anyPitchedSampling = validMaps.some((entry) => Math.max(1, Number(entry.rowPitch || 1)) > 1 || Math.max(1, Number(entry.colPitch || 1)) > 1);
    return hasOffsetVariants && allDefaultPlane && anyPitchedSampling;
}

function readBigEndianU32(bytes, offset) {
    const source = normalizeByteArray(bytes);
...
function readBigEndianF32(bytes, offset) {
    const source = normalizeByteArray(bytes);
    if (offset < 0 || offset + 4 > source.length) return 0;
...
function readBigEndianF64(bytes, offset) {
    const source = normalizeByteArray(bytes);
    if (offset < 0 || offset + 8 > source.length) return 0;
    const view = new DataView(source.buffer, source.byteOffset, source.byteLength);
    return view.getFloat64(offset, false);
}

function parseGainMapOpcodeBody(bytes) {
    const source = normalizeByteArray(bytes);
    if (source.length < 76) return null;
    const top = readBigEndianU32(source, 0);
    const left = readBigEndianU32(source, 4);
    const bottom = readBigEndianU32(source, 8);
    const right = readBigEndianU32(source, 12);
    const plane = readBigEndianU32(source, 16);
    const planes = Math.max(1, readBigEndianU32(source, 20));
    const rowPitch = Math.max(1, readBigEndianU32(source, 24));
    const colPitch = Math.max(1, readBigEndianU32(source, 28));
    const mapPointsV = Math.max(1, readBigEndianU32(source, 32));
    const mapPointsH = Math.max(1, readBigEndianU32(source, 36));
    const mapSpacingV = readBigEndianF64(source, 40);
    const mapSpacingH = readBigEndianF64(source, 48);
    const mapOriginV = readBigEndianF64(source, 56);
    const mapOriginH = readBigEndianF64(source, 64);
    const mapPlanes = Math.max(1, readBigEndianU32(source, 72));
    const gainCount = mapPointsV * mapPointsH * mapPlanes;
    const requiredBytes = 76 + (gainCount * 4);
    if (source.length < requiredBytes) return null;

    const gains = new Float32Array(gainCount);
    if (mapPlanes === 1) {
        for (let index = 0; index < gainCount; index += 1) {
            gains[index] = readBigEndianF32(source, 76 + (index * 4));
        }
    } else {
        const mapPointsTotal = mapPointsV * mapPointsH;
        for (let point = 0; point < mapPointsTotal; point += 1) {
            for (let planeIdx = 0; planeIdx < mapPlanes; planeIdx += 1) {
                const sourceIndex = point * mapPlanes + planeIdx;
                const destIndex = planeIdx * mapPointsTotal + point;
                gains[destIndex] = readBigEndianF32(source, 76 + (sourceIndex * 4));
            }
        }
    }
...
export function parseOpcodeList(value) {
    ...
    const count = readBigEndianU32(bytes, 0);
    const entries = [];
    ...
    for (let index = 0; index < count; index += 1) {
        ...
        const opcodeId = readBigEndianU32(bytes, offset);
        const version = readBigEndianU32(bytes, offset + 4);
        const flags = readBigEndianU32(bytes, offset + 8);
        const size = readBigEndianU32(bytes, offset + 12);
        const bodyStart = offset + 16;
        const bodyEnd = Math.min(bytes.length, bodyStart + size);
        const body = bytes.slice(bodyStart, bodyEnd);
        let parsed = null;
        let kind = 'unknown';

        if (opcodeId === 9) {
            parsed = parseGainMapOpcodeBody(body);
            kind = 'gain-map';
        } else {
            unsupportedIds.push(opcodeId);
        }
...
export function decodeParsedDngMetadata(parsed) {
    const rawIfd = parsed?.rawIfd || {};
    const tags = rawIfd.tags || {};
    const opcodeList1 = parseOpcodeList(tags.OpcodeList1);
    const opcodeList2 = parseOpcodeList(tags.OpcodeList2);
    const opcodeList3 = parseOpcodeList(tags.OpcodeList3);
    const gainMaps = [
        ...opcodeList1.entries,
        ...opcodeList2.entries,
        ...opcodeList3.entries
    ]
        .filter((entry) => entry.kind === 'gain-map' && entry.parsed)
        .map((entry) => entry.parsed);

    return {
        blackLevelRepeatDim: toArray(tags.BlackLevelRepeatDim).map((value) => clampPositiveInteger(value, 0)).filter(Boolean),
        blackLevel: toArray(tags.BlackLevel).map((value) => Number(value) || 0),
        whiteLevel: toArray(tags.WhiteLevel).map((value) => Number(value) || 0),
        asShotNeutral: toArray(tags.AsShotNeutral).map((value) => Number(value) || 0),
        colorMatrix1: toArray(tags.ColorMatrix1).map((value) => Number(value) || 0),
        colorMatrix2: toArray(tags.ColorMatrix2).map((value) => Number(value) || 0),
        analogBalance: toArray(tags.AnalogBalance).map((value) => Number(value) || 0),
        cameraCalibration1: toArray(tags.CameraCalibration1).map((value) => Number(value) || 0),
        cameraCalibration2: toArray(tags.CameraCalibration2).map((value) => Number(value) || 0),
        forwardMatrix1: toArray(tags.ForwardMatrix1).map((value) => Number(value) || 0),
        forwardMatrix2: toArray(tags.ForwardMatrix2).map((value) => Number(value) || 0),
        noiseProfile: toArray(tags.NoiseProfile).map((value) => Number(value) || 0),
        defaultCropOrigin: toArray(tags.DefaultCropOrigin).map((value) => Number(value) || 0),
        defaultCropSize: toArray(tags.DefaultCropSize).map((value) => Number(value) || 0),
        activeArea: toArray(tags.ActiveArea).map((value) => Number(value) || 0),
        linearizationTable: normalizeByteArray(tags.LinearizationTable).length
            ? toArray(tags.LinearizationTable).map((value) => Number(value) || 0)
            : [],
        opcodeList1,
        opcodeList2,
        opcodeList3,
        gainMaps,
        profileGainTableMap: normalizeByteArray(tags.ProfileGainTableMap)
    };
}
...
function classifyProbe(rawIfd, rootIfd) {
    ...
    const decoded = decodeParsedDngMetadata({ rawIfd, rootIfd });
    ...
    const hasOpcodeList = !!(tags.OpcodeList1 || tags.OpcodeList2 || tags.OpcodeList3);
    const hasProfileGainTableMap = !!tags.ProfileGainTableMap;
    const opcodeEntries = [
        ...decoded.opcodeList1.entries,
        ...decoded.opcodeList2.entries,
        ...decoded.opcodeList3.entries
    ];
    const hasStandaloneOpcodeMetadata = hasOpcodeList && opcodeEntries.some((entry) => entry?.kind !== 'gain-map');
    const hasStandaloneExecutableOpcodes = opcodeEntries.some((entry) => entry?.kind !== 'gain-map' && entry?.parsed);
    const hasOnlyGainMapOpcodes = hasOpcodeList
        && opcodeEntries.length > 0
        && opcodeEntries.every((entry) => entry?.kind === 'gain-map' && entry?.parsed);
    const phaseAwareGainMapLayout = usesPhaseAwareGainMapLayout(decoded.gainMaps);
    const gainMapRuntimeSupported = decoded.gainMaps.length > 0 && !hasProfileGainTableMap && !phaseAwareGainMapLayout;
    const warnings = [];
    let fidelity = 'supported';
    const unsupportedOpcodeIds = [
        ...decoded.opcodeList1.unsupportedIds,
        ...decoded.opcodeList2.unsupportedIds,
        ...decoded.opcodeList3.unsupportedIds
    ];
    ...
    if (hasOpcodeList && unsupportedOpcodeIds.length) {
        fidelity = fidelity === 'unsupported' ? fidelity : 'partial';
        warnings.push(`This DNG includes unsupported opcode correction IDs: ${[...new Set(unsupportedOpcodeIds)].join(', ')}.`);
    }
    if (hasStandaloneOpcodeMetadata && !unsupportedOpcodeIds.length && !hasStandaloneExecutableOpcodes && !hasOnlyGainMapOpcodes) {
        fidelity = fidelity === 'unsupported' ? fidelity : 'partial';
        warnings.push('This DNG carries opcode metadata, but standalone opcode execution is not implemented in this build.');
    }
    ...
    } else if (phaseAwareGainMapLayout) {
        fidelity = fidelity === 'unsupported' ? fidelity : 'partial';
        warnings.push('This DNG uses phase-aware gain maps. That correction path is disabled in the current build because it is not color-stable on current phone test files.');
    }
    return {
        ...
        analogBalance: decoded.analogBalance,
        cameraCalibration1: decoded.cameraCalibration1,
        cameraCalibration2: decoded.cameraCalibration2,
        forwardMatrix1: decoded.forwardMatrix1,
        forwardMatrix2: decoded.forwardMatrix2,
        ...
        hasLinearizationTable: decoded.linearizationTable.length > 0,
        hasOpcodeList,
        hasStandaloneOpcodeMetadata,
        hasOnlyGainMapOpcodes,
        hasGainMap: hasProfileGainTableMap || decoded.gainMaps.length > 0,
        hasProfileGainTableMap,
        usesPhaseAwareGainMaps: phaseAwareGainMapLayout,
        supportsLinearization: decoded.linearizationTable.length > 0,
        supportsOrientationToggle: clampPositiveInteger(tags.Orientation, 1) !== 1,
        supportsRemosaic: classificationMode === 'quad' || classificationMode === 'interleaved',
        supportsOpcodeCorrections: hasStandaloneExecutableOpcodes && unsupportedOpcodeIds.length === 0,
        supportsGainMapCorrections: gainMapRuntimeSupported,
        fidelity,
        warnings
    };
}
